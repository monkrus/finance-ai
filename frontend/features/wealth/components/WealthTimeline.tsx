...
import { useWealthTimeline } from '../api/queries';
...
export function WealthTimeline() {
  const { data, isLoading, isError, refetch } = useWealthTimeline();
            ...
            {data.map((event, index) => (
              ...
              >
                <div className={cn(
                  ...
                  event.type === 'milestone' ? 'text-indigo-500' :
                  event.type === 'transaction' ? 'text-emerald-500' :
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <h4 className="text-sm font-semibold group-hover:text-indigo-400 transition-colors">{event.title}</h4>
